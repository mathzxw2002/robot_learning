...
Pi AI 发布了一篇关于“实时分块”（Real-Time Chunking）技术的文章和论文↓视频演示了有RTC加持和无RTC的延时对比。

★问题背景：

在机器人领域，与聊天机器人或图像生成器不同，机器人必须在实时环境下操作。这意味着，机器人在“思考”时，周围世界仍按物理规律运行，输入与输出之间的延迟会直接影响性能。语言模型的快慢可能仅影响用户满意度，但对于视觉-语言-动作（VLA）模型，差别可能是成功递上一杯热咖啡或是将其洒在你腿上。

★ 解决方案：
要构建实时系统，我们需要某种异步形式：即让模型在执行前一个动作时就考虑其未来动作。动作分块（Action Chunking）提供了一个不错的起点。

VLA使用的是每次推理调用执行50个动作序列，对应实际时间中的1秒。然而，仅有分块还不够，切换分块时新旧动作可能不一致，导致不安全的加速度。

Pi AI 开发了一种称为实时分块（RTC）的算法，能够在无需训练时更改的情况下，实现无不连续性的实时执行，并适用于任何基于扩散或流的VLA模型。RTC显著加快了所有测试任务的执行时间，对延迟也非常鲁棒，即使在人为引入超过300毫秒的延迟时也能完成动态精确任务。

通过将实时分块视为一个涂鸦问题来解决动作块之间一致性的核心挑战。通过适应性算法和“部分关注”概念，项目解决了块一致性问题，无需训练时变更。此外，通过一系列实验，发现与同步推理相比，RTC不仅加快了执行速度，还提高了精度。

Physical Intelligence的这项研究为实时机器人系统提供了有效且简单的解决方案，展现了即使在高延迟下也有出色韧性的策略。随着模型规模的增加，实时推理的准确性和效率将变得越来越重要。

访问：www.pi.website/research/real_time_chunking
...
这是一款将被部署到家庭中的视觉-语言模型（vision-language transformer），专为人形机器人量身打造，能够执行从头到尾的移动操控任务，如为用户取物、开门以及在家中导航。

1. 特性概览：
   - 泛化能力：可以处理任务变化，比如在不熟悉的位置捡起从未见过的物体。通过在EVE和NEO上收集的大量远程操作和自主操作的数据集训练，Redwood展现了如选择行为这样的突发行为。

   - 全身和多点接触操控：Redwood是首批能够同时控制行走和操控的视觉-语言应用之一，使其在操控过程中能够支撑和倾斜。

   - 移动双手操控：允许NEO精确定位执行任务，执行需要跨空间移动的动作，并在移动中操控物体。

   - 完全板载运行：Redwood计算高效，完全运行在NEO的板载嵌入式GPU上。

2. 跨载体架构：
   Redwood通过融合预训练的语言嵌入、视觉变换器中的视觉标记、以及一系列关节位置和施加的关节力量的本体感知嵌入，通过若干Transformer块提取潜在表示向量，再将这一表示解码为EVE或NEO的动作，采用扩散策略。

3. 全身控制和多点接触操控：
   在大多数机器人系统中，操控和行走行为通常是分开的。但在家庭环境中，操控任务需要超越在台面上捡小物品：人类使用腿、臀和脊椎弯腰捡地上的玩具和衣物，推开重门时向门倾斜。Redwood不仅预测手臂和手的指令，还同时预测走路、操控和骨盆姿势指令，大大扩展了NEO可以操作的动作范围和负载能力。
...
Cursor 出了一期圆桌视频，创始成员和研究人员探讨了 RL 应用于代码生成模型的挑战、长下文、工具在 RL 中的角色等议题。

链接：www.youtube.com/watch?v=sLaxGAL_Pl0

以下要点由 Gemini 2.5 Pro 整理：

1. RL用于代码生成的特殊性与挑战：

🌟 动作空间巨大：与数学等领域相比，编码的可能动作（生成的代码序列）非常庞大。

🌟 推理与答案的关系：
- 数学：推理过程帮助模型得到简短的最终答案，RL可以奖励这个推理过程。
- 编码：代码本身即是推理过程和答案的结合，奖励直接针对生成的代码。

🌟工具使用（Tool Use）：
- 编码任务常需多步调用外部工具（如Linter、搜索、终端执行）。
- RL过程变为：生成Token -> 调用工具 -> 获取工具响应 -> 迭代 -> 最终获得奖励。这使得RL的“形状”与简单问答不同，需要优化整个工具调用链。

🌟 奖励信号与验证困难：
- 许多真实代码任务不像数学题或有单元测试的代码问题那样有“黄金标准答案”。
- 希望在用户不总提供明确反馈（如点赞/点踩）的情况下也能应用RL。
- “通过测试”作为奖励信号有局限性，模型可能通过“hack”手段通过测试，其生成过程可能与预期不符，且代码质量、优雅性、简洁性难以衡量。

2. RL在不同领域的应用对比与思考：

🌟 写作领域：
- 当前模型写作风格可能生硬、正式，但这更多是训练方式导致，而非模型固有缺陷。
- RL用于写作的挑战：评价主观，好坏是品味问题。
- RL思路：可训练模型预测整个序列（如下一章节），而非仅下一词元，并使用语义相似度等作为奖励。下一词元预测无法完全捕捉生成连贯长文本的目标。

🌟 奖励信号的普遍问题：
- 需要更接近用户真实需求的奖励信号（“用户的微笑”）。
- 对奖励模型的RLHF（从人类反馈中强化学习）容易饱和或过拟合奖励模型本身（通常几百步），而有真实Ground Truth的RL（如DeepMind的AlphaCode或DeepSeek Coder的数学/代码解题）可以进行更长时间的优化（如上万步）。

3. 长上下文（Long Context）的重要性与实现：

🌟 对编码至关重要：理解和生成大型代码库中的代码需要长上下文能力，关键上下文长度可能达50-60k tokens甚至更多。

🌟 趋势与成本：上下文窗口越来越长，但标准注意力机制的计算成本（O(N^2)）随之剧增。

🌟 优化方向：
- 智能缓存和重用上下文（如预计算KV缓存）。
- 新注意力架构：如DeepMind的NSA（结合滑动窗口、分块注意力、Top-K选定块的全注意力）和Nvidia的GB200 NVL72（通过NVLink互联72个GPU，支持大规模张量并行和统一内存）等硬件支持。
- 混合机制：可能需要模型能对大规模上下文进行粗略理解，然后对关键部分进行精细处理和记忆刷新。

4. 工具（Tools）在RL中的角色：

🌟 现有工具类型：
- 终端：简单直接，但O3模型有时过度依赖（如用grep和sed）。
- Linter：提供有用信号，但设置和运行语言服务器较复杂。
- 搜索：语义搜索比grep快，但可能损失精确信息。
- 编辑工具。

🌟 记忆工具（Memory Tool）：
- 分为“存储记忆”和“检索记忆”两部分。
- 检索的奖励相对直接：如果检索到的记忆帮助解决了当前问题，则给予奖励。
- 存储的奖励更复杂：当前存储的记忆是否有用，依赖于未来的交互轨迹。
- 非模型驱动的记忆生成和检索可能在短期内更易实现。

🌟 工具质量：希望模型能使用高质量工具，或能自主选择和优化工具使用。

5. RL基础设施（Infrastructure）：

🌟 复杂性：RL基础设施天然比监督微调（SFT）或预训练更复杂，因为它建立在它们之上，并增加了推理、奖励计算等环节。

🌟 优化重点：
- 推理组件需为吞吐量优化（RL训练时），而非用户交互时的低延迟。
- 通过异步rollouts、共享prompt的KV缓存预计算等方式提高效率。
- 训练节点与推理节点之间的参数需要快速同步。

6. 对未来编码智能体的展望：

🌟 将使用更多tokens（尤其在输出端，形成更完整的“思考”过程或代码块）。

🌟 能更有效地利用代码库中的海量相关上下文。

🌟 可能依赖于更先进的长上下文处理技术和更智能的工具使用。

🌟 奖励信号会更贴近用户真实需求和偏好，可能包括对代码质量、可维护性等的隐式或显式评估。



【[25星]Gemma3 Object Detection：让Gemma 3模型在目标检测任务上大放异彩。亮点：1. 使用特殊`<locXXXX>`标签，将目标位置融入语言模型词汇表；2. 数据增强助力模型性能提升；3. 提供完整的训练和推理流程，易于上手】  
'Fine tune Gemma 3 on an object detection task'  
GitHub: github.com/ariG23498/gemma3-object-detection  
